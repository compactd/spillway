...
describe('Up/Down stream wire', () => {
  test('addTorrent', async () => {
    ...
    const { client, server } = await createTestWires({ addTorrent });
  ...
  test('getState', async () => {
    ...
    const { client, server } = await createTestWires({ getState });
  ...
  test('getPiece', async () => {
    ...
    const { client, server } = await createTestWires({ getPiece });
